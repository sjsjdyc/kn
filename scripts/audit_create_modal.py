...
def render_kind(src: str, state: str) -> str:
    ...
    for p in pola:
        for m in re.finditer(p, src):
            ...
            kinds.append("modal" if (any(mk in blok for mk in MODAL_MARKERS)
                                     or MODAL_COMPONENT.search(blok)) else "inline")
    ...
    for rel, src in files:
        ...
        for setter, state in setters.items():
            ...
            if render_kind(src, state) != "inline":
                continue
            # hanya hitung bila blok yang dibuka memang FORM (ada input/field), supaya
            # panel/informasi yang dibuka-tutup tidak dituduh sebagai form create.
            m2 = re.search(rf"\{{\s*{re.escape(state)}\s*&&", src)
            blok = src[m2.start(): m2.start() + 2500] if m2 else ""
            if not re.search(r"<input|className=\"field\"|<textarea|<KNSelect", blok):
                continue
